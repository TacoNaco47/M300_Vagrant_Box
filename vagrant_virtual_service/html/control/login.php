...
if(isset($_POST['email']) && isset($_POST['passwort'])) {
	...
	if ($user !== false && password_verify($passwort, $user['passwort'])) {
		...
		if(isset($_POST['angemeldet_bleiben'])) {
			...
			setcookie("identifier",$identifier,time()+(3600*24*365)); //Gültigkeit für ein Jahr
			setcookie("securitytoken",$securitytoken,time()+(3600*24*365)); //Gültigkeit für ein Jahr
		}

		header("location: internal_area.php");
		exit;
	} else {
